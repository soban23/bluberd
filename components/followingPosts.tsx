...
interface Props {
    ...
    typeOfPosts?: string | null;//following or popular
...
export default function FollowingPosts({ typeOfPosts = null, forComment_user_id = null, forPost_user_id = null, post_id = null, comment_id = null, newContent, onConsume = () => { } }: Props) {
    ...
    // const [selectedPostIdToComment, setSelectedPostIdToComment] = useState<string | null>(null);
    ...
    useEffect(() => {
        ...
        const processNewComment = async () => {
            try {
                ...
                console.log('comments after insert', normalizedComment);
    ...
    //check realtime add comms/ comms liles stats wrong
    ...
    const fetchUserCommentLikes = async () => {
        try {
            ...
            console.log('data -- ', data)
            console.log('liekdopsts -- ', likedComments)
        } catch (error) {
            console.error('Failed to fetch user likes', error);
        }
    ...
    const handleLikesClick = ({ postId = null, commentId = null }: { postId?: string | null, commentId?: string | null }) => {
        if (commentId) {
    ...
    const handleCommentClick = ({ postId = null, commentId = null }: { postId?: string | null, commentId?: string | null }) => {
        const id = postId || commentId;

        if (showCommentBox !== id) {
            setCommentContent(""); // Clear textarea if switching
            setShowCommentBox(id);
        } else {
            setShowCommentBox(null); // Toggle off if clicked again
        }


    };

    const handleClick = ({ postId = null, commentId = null }: { postId?: string | null, commentId?: string | null }) => {
        if (commentId) {
    ...
    const handleCommentSubmit = async (e: FormEvent, { postId = null, commentId = null }: { postId?: string | null, commentId?: string | null }) => {
        ...
        if (res.ok) {
            ...
            if (commentId) {
                setComments((prev) =>
                    prev.map((p) =>
                        p.commentid === commentId ? { ...p, numberofcomments: Number(p.numberofcomments) + Number(1) } : p
                    )
                );
                ...
                setPosts((prev) =>
                    prev.map((p) =>
                        p.postid === postId ? { ...p, numberofcomments: Number(p.numberofcomments) + Number(1) } : p
                    )
                );
    ...
    const deleteLike = async ({ postId = null, commentId = null }: { postId?: string | null, commentId?: string | null }) => {
        ...
        try {
            let res;
            if (commentId) {
                res = await fetch(`/api/comments/${commentId}/likes`, {//adjust
                    method: 'DELETE',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({}),
                })
    ...
    const toggleLike = ({ postId = null, commentId = null }: { postId?: string | null, commentId?: string | null }) => {
        if (commentId) {
            ...
            if (isLiked) {
                ...
                setComments((prev) =>
                    prev.map((p) =>
                        p.commentid === commentId ? { ...p, numberoflikes: p.numberoflikes - 1 } : p
                    )
                );
            } else {
                ...
                setComments((prev) =>
                    prev.map((p) =>
                        p.commentid === commentId ? { ...p, numberoflikes: Number(p.numberoflikes) + Number(1) } : p
                    )
                );
            }
            ...
            if (isLiked) {
                ...
                setPosts((prev) =>
                    prev.map((p) =>
                        p.postid === postId ? { ...p, numberoflikes: p.numberoflikes - 1 } : p
                    )
                );
            } else {
                ...
                setPosts((prev) =>
                    prev.map((p) =>
                        p.postid === postId ? { ...p, numberoflikes: Number(p.numberoflikes) + Number(1) } : p
                    )
                );
            }
    ...
    const handleDelete = async ({ postId = null, commentId = null }: { postId?: string | null, commentId?: string | null }) => {

        try {
            let res;
            if (commentId) {
                res = await fetch(`/api/comments/${commentId}`, {//adjust
                    method: 'DELETE',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({}),
                })
    ...
    //     <div>
    //         {loading ? (
    //             <div className='flex items-center justify-center'><Spinner></Spinner></div>
    ...
    //                             onLikesClick={() => handleLikesClick({ commentId: comment.commentid })}
    //                             onComment={() => handleCommentClick({ commentId: comment.commentid })}
    //                             onCommentsClick={() => handleCommentClick({ commentId: comment.commentid })}
    ...
    //                             <div className="w-full mx-auto max-w-xl mt-[0px]">
    //                                 <div className="relative border rounded-md shadow-sm bg-white px-4 py-3">
    //                                     <button
    ...
    //                                     </button>
    //                                     <form
    //                                         onSubmit={(e) => handleCommentSubmit(e, { commentId: comment.commentid })}
    ...
    //                                             className="text-sm border rounded-md p-2 resize-none h-20 w-full focus:outline-none focus:ring-1 focus:ring-primary"
    //                                         />
    //                                         <button
    //                                             type="submit"
    //                                             disabled={loading}
    ...
    //                             onCommentsClick={() => handleCommentClick({ postId: post.postid })}
    ...
    //                             <div className="w-full max-w-xl mx-auto mt-[0px]">
    //                                 <div className="relative border rounded-md shadow-sm bg-white px-4 py-3">
    //                                     <button
    //                                         onClick={() => setShowCommentBox(null)}
    //                                         className="absolute top-2 right-3 text-sm text-gray-400 hover:text-red-500"
    ...
    //                                     </button>
    //                                     <form
    //                                         onSubmit={(e) => handleCommentSubmit(e, { postId: post.postid })}
    //                                         className="flex flex-col gap-2"
    //                                     >
    //                                         <textarea
    //                                             name="message"
      ...
      <>
        {comments.map((comment) => (
          <div key={comment.commentid}>
            ...
            {showLikesModal && selectedCommentId === comment.commentid && (
              <LikesModal commentId={selectedCommentId} onClose={() => setShowLikesModal(false)} />
            )}
          </div>
        ))}
      </>
      ...
      <>
        {posts.map((post) => (
          <div key={post.postid}>
            ...
            {showLikesModal && selectedPostId === post.postid && (
              <LikesModal postId={selectedPostId} onClose={() => setShowLikesModal(false)} />
            )}
