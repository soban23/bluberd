...
import { Avatar, AvatarImage, AvatarFallback } from '@/components/ui/avatar'; // If using shadcn
// Or create your own <Avatar> component
...
export default function BottomNav() {
  ...
  <nav className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-50 bg-white dark:bg-gray-900 shadow-lg rounded-full px-6 py-2 w-[90%] max-w-md flex justify-between items-center backdrop-blur-md border dark:border-gray-700">
    <NavItem icon={<Home onClick={() => router.push('/home')} className="w-6 h-6" />} />
    <NavItem icon={<Search onClick={() => router.push('/search')} className="w-6 h-6" />} />
    <NavItem icon={<MessageCircle onClick={() => router.push(`/conversations`)} className="w-6 h-6" />} />
    <NavItem
      icon={
        <Avatar onClick={() => router.push(`/profile/${session.user.id}`)} className="w-7 h-7">
