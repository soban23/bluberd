    ...
    <div>
      {loading ? (
        ...
        <div className="cursor-default lg:mx-[120px] xl:mx-[180px] 2xl:mx-[240px]  ">
          <div className="grid grid-cols-[1fr_3fr] gap-4 min-h-[48px] text-lg">
          ...
          {conversations.map((convo) => (
            <div key={convo.conversation_id}>
              {convo.latestmessage && (
                <div className="grid grid-cols-[1fr_3fr] gap-4 min-h-[96px]">
                  <div className="flex items-center justify-center w-full h-full">
                    {session?.user.id != convo.user1_id && (
                      <div>
                        <Avatar
                          onClick={() =>
                            router.push(`profile/${convo.user1_id}`)
                          }
                        >
                    ...
                    {session?.user.id != convo.user2_id && (
                      <div>
                        <Avatar
                          onClick={() =>
                            router.push(`profile/${convo.user2_id}`)
                          }
                        >
                  ...
                  >
                    ...
                    <span className="text-sm text-muted-foreground">
                      {convo.messagetime &&
                      !isNaN(new Date(convo.messagetime).getTime())
                        ? formatDistanceToNow(new Date(convo.messagetime), {
                            addSuffix: true,
                          })
                        : "Invalid time"}
                    </span>
              ...
              {convo.latestmessage && <Separator></Separator>}
            </div>
          ))}
        </div>
      )}
