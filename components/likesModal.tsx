...
export default function LikesModal({ postId = null, commentId = null, onClose }: LikesModalProps) {
  ...
  const fetchLikes = async () => {
    ...
    try {
      ...
      if (commentId && !postId) {
        ...
        console.warn('Only one of postId or commentId should be passed.');
        return;
      }
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
    <Card className="w-full max-w-md rounded-lg shadow-md bg-white dark:bg-gray-900 max-h-[80vh] overflow-hidden">
      <CardHeader className="flex items-center justify-between border-b px-4 py-3 border-gray-200 dark:border-gray-700">
        <CardTitle className="text-base text-black dark:text-white">Liked by</CardTitle>
        <button
          onClick={onClose}
          ...
          <div className="max-h-[400px] overflow-y-auto space-y-2">
            {likes.map((like) => (
              ...
              >
                ...
                <div>
                  <p className="font-medium text-black dark:text-white">{like.name}</p>
                  {/* <p className="text-xs text-gray-500 dark:text-gray-400">@{like.username}</p> */}
                </div>
              </div>
            ))}
          </div>
