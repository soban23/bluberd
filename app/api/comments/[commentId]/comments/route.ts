import { createComment, getCommentsOfCommentLead } from '@/lib/db/comments';
...
export const GET = async (req: Request, context:any) => {
    try {
        ...
        if (!commentId ) {
            return new Response('commentId missing', { status: 401 });
        }
        ...
        const comments = await getCommentsOfCommentLead(commentId);
...
export const POST = async (req: Request, context:any) => {
    try {
        ...
        const comment = await createComment(content, postId, userId, commentId);
